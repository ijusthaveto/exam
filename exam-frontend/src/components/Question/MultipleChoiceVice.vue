<template>
  <div class='center-container'>
    <el-table :data="filterTableData" style="width: 100%" height='672px' >
      ...
      <el-table-column label='Answer' >
        <template #default='scope'>
          <span v-for='item in JSON.parse(scope.row.correctAnswer)' :key='item'>{{item}}&nbsp;</span>
        </template>
      </el-table-column>
  ...
  <el-dialog
      ...
      align-center
      title="Modify Multiple Question Information"
      width="30%"
  >
    <div>
      <el-form
...
const getMultipleQuestionList = async () => {
